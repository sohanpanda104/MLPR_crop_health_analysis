...
# ========== CONFIG ========== #
CSV_PATH = "../combined_dataset/final_datasets/Final_dataset_april_14.csv"
IMAGE_FOLDER = "/home/bhavya/Downloads"
IMG_SIZE = (128, 128)
NUM_CLASSES = 4
BATCH_SIZE = 8
CHANNELS = 10
...
class_counts = original_df['category'].value_counts()
max_count = class_counts.max()
balanced_df = pd.DataFrame()

for cat in class_counts.index:
    subset = original_df[original_df['category'] == cat]
    repeat_factor = int(np.ceil(max_count / len(subset)))
    oversampled = pd.concat([subset] * repeat_factor, ignore_index=True)
    balanced_df = pd.concat([balanced_df, oversampled], ignore_index=True)
...
# ========== Save Model ========== #
model.save("crop_health_generator_model3.h5")
print("\n✅ Model saved as crop_health_generator_model3.h5")
